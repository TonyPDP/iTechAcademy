...
import Navbar from "../Navbar/Navbar";
import SuccessModal from "./Modal";
// Agar public papkadan rasm ishlatilsa:
const Mentor = () => {
  const [showModal, setShowModal] = useState(false);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8 font-semibold">
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        />
        <div>
          <p className="font-semibold text-sm text-gray-500 mb-2">We only support .JPG, .JPEG, or PNG FILE</p>
          <div className="flex gap-4">
            <button className="border border-green-700 text-green-700 bg-green-100 px-3 py-1 rounded underline">Upload Photo</button>
            <button className="text-red-600">Delete Photo</button>
          </div>
        </div>
      </div>
      ...
      <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSave}>
        ...
        <div className="col-span-2 flex justify-end gap-3 mt-6">
          <button type="button" className="font-semibold px-4 py-2 border border-red-600 text-red-600 rounded">
            Back
          </button>
        </div>
      </form>

      {showModal && <SuccessModal onClose={handleCloseModal} onAddAgain={handleAddAgain} />}
    </div>
  );
};
